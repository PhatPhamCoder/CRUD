      ...
      conn.query(
        ...
        async (err, dataRes) => {
          try {
            ...
            if (
              dataRes[0]?.expired_on < Date.now() / 1000 &&
              dataRes[0]?.expired_on !== null
            ) {
              return result(
                {
                  param: "password",
            ...
            if (!passMatch) {
              return result(
                { param: "password", msg: constantNotify.PASS_FAILED },
                null,
              );
            }
    ...
    db.getConnection((err, conn) => {
      ...
      conn.query(
        ...
        async (err, dataRes) => {
          ...
          let passwordNew = "";
          for (let i = 0; i < 3; i++) {
            passwordNew += stringA_Z.charAt(
              Math.floor(Math.random() * stringA_Z.length),
            );
          }
          for (let i = 0; i < 3; i++) {
            passwordNew += stringa_z.charAt(
    ...
    db.getConnection((err, conn) => {
      ...
      conn.query(
        ...
        async (err, dataRes) => {
          ...
          if (!passMatch) {
            return result(
              { param: "password", msg: constantNotify.PASS_OLD_FAILED },
              null,
            );
          }
          const salt = await bcrypt.genSalt(12);
